...
Scenario Outline: Access Aml Report as company admin
  ...
  Given there is an investor "" with investor kyc and aml report for the entity "<entity>"
...
Scenario Outline: Access Aml Report as Other User
  ...
  Then "<another_user>" has "false" "<crud>" access to the aml_report of investor 

  Examples:
  |user	               |entity               |role |crud|another_user|
  |first_name=AdminUser|entity_type=Investment Fund;name=TestCompany877  |company_admin|"index,show,create,toggle_approved,generate_new"|first_name=Investor21|
  |first_name=approver  |entity_type=Investment Fund;name=TestCompany867  |approver |"index,show,create,toggle_approved,generate_new"|first_name=Investor22|
  |first_name=signatory  |entity_type=Investment Fund;name=TestCompany887  |signatory |"index,show,create,toggle_approved,generate_new"|first_name=Investor23|

Scenario Outline: Auto Create new Aml Report
  Given Im logged in as a user "<user>" for an entity "<entity>"
  Given the user has role "company_admin"
  Given the entity "<entity>" has aml enabled "true"
  Then there is an investor "" for the entity "<entity>" with investor kyc and aml report is generated for it
...
Scenario Outline: Do not Create new Aml Report with blank name
  ...
  Given the user has role "company_admin"
  Given the entity "<entity>" has aml enabled "true"
  Then there is an investor "" for the entity "<entity>" with investor kyc and aml report is not generated for it
