...
(function (global, factory) {
    ...
    formBuilders: {
      'ActionView::Helpers::FormBuilder': {
        ...
        remove: function remove($element, settings) {
          ...
          let cf = $(element).hasClass("custom_field");
          if(!cf) {
            if (inputErrorElement) {
              console.log(`Removing error for ${element.id}`);
              if(element.id) {
                // Modified by Mohith
  ...
  var runFunctionValidations = function runFunctionValidations(formattedValue, form, options) {
    for (var validation in VALIDATIONS$1) {
      ...
      console.log(`Validating: ${validationFunction} with ${validationOption}`);
  ...
  var isMarkedForDestroy = function isMarkedForDestroy($element) {
    // Modified by Mohith
    if ($element.attr('name') && $element.attr('name').search(/\[([^\]]*?)\]$/) >= 0) {
