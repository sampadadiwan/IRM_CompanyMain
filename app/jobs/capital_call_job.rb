  ...
  def perform(capital_call_id)
    ...
    @capital_call.fund.capital_commitments.each do |cc|
      ...
      if CapitalRemittance.exists?(capital_call_id: @capital_call.id, investor_id: cc.investor.id)
        logger.debug "Skipping as CapitalRemittance exists for #{cc.investor.investor_name} for #{@capital_call.name}"
      else
        logger.debug "Creating CapitalRemittance for #{cc.investor.investor_name} for #{@capital_call.name}"
