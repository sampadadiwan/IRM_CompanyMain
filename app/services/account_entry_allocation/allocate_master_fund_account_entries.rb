  ...
  class AllocateMasterFundAccountEntries < AllocationBaseOperation
    ...
    def allocate_master_fund_account_entries(ctx, name_or_entry_type:, **)
      ...
      fund_formula.commitments(end_date, sample).each_with_index do |capital_commitment, idx|
        master_fund_entry = master_fund_account_entries_cache[capital_commitment.unit_type]

        if master_fund_entry.blank?
          ...
          master_fund_entry = commitment_in_master.account_entries.where(
          ...
          master_fund_account_entries_cache[capital_commitment.unit_type] = master_fund_entry
        end

        Rails.logger.debug { "Allocating #{master_fund_entry} to #{capital_commitment}" }

        commitment_cache.computed_fields_cache(capital_commitment, start_date)

        # Create a local copy to insert
        ae = master_fund_entry.dup
        ae.fund_id   = fund.id
        ...
        begin
          ...
          AccountEntryAllocation::CreateAccountEntry.wtf?(ctx.merge(account_entry: ae, capital_commitment: capital_commitment, parent: master_fund_entry, bdg: binding))
        rescue StandardError => e
          raise "Error in #{fund_formula.name} for #{capital_commitment} #{master_fund_entry}: #{e.message}"
        end
