    ...
    def allocate_aggregate_portfolios(ctx, **)
      ...
      fund_formula.commitments(end_date, sample).each_with_index do |capital_commitment, idx|
        ...
        apis.each do |orig_api|
          ae = AccountEntry.new(
            name: "#{orig_api.portfolio_company_name}-#{orig_api.investment_type}",
          ...
          begin
            ...
            AccountEntryAllocation::CreateAccountEntry.wtf?(ctx.merge(account_entry: ae, capital_commitment: capital_commitment, parent: orig_api, bdg: binding))
