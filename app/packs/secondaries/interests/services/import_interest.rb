class ImportInterest < ImportUtil
  STANDARD_HEADERS = ["Buyer Entity Name", "Address", "Contact Name", "City", "Pan", "Demat", "Bank Account", "Ifsc Code", "Buyer Signatory Emails", "Investor", "Quantity", "Price", "Short Listed Status", "Escrow Deposited", "Update Only"].freeze
  ...
  def save_row(user_data, import_upload, custom_field_headers, ctx)
    ...
    email, update_only, user, short_listed_status, escrow_deposited, investor = key_data(user_data, import_upload)
    ...
    if update_only
      ...
      interest = Interest.find_by(entity_id: import_upload.entity_id, investor_id: investor.id, secondary_sale_id: secondary_sale.id, id: user_data["Interest Id"])
