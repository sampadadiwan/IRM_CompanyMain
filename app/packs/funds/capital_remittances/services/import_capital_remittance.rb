class ImportCapitalRemittance < ImportUtil
  ...
  def save_capital_remittance(user_data, import_upload, custom_field_headers)
    ...
    if fund && capital_call && investor && capital_commitment
      ...
      capital_remittance = CapitalRemittance.new(entity_id: import_upload.entity_id, fund:, capital_call:, investor:, investor_name: investor.investor_name, capital_commitment:, status: user_data["Status"], folio_id:, collected_amount_cents:, folio_call_amount: user_data["Due Amount"], fee: user_data["Fees"], payment_date: user_data["Payment Date"], created_by: "Upload")
