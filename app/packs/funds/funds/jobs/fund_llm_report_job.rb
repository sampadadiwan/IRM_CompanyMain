class FundLlmReportJob < ApplicationJob
  ...
  def perform(fund_id, user_id, report_type,
    ...
    Chewy.strategy(:sidekiq) do
      ...
      when "AggregatePortfolioInvestment"
        api_reports(fund, user_id, report_template_name, include_kpis:, include_apis:, start_date:, end_date:)
      else
    ...
    if kpi_reports.present?
      ...
      kpi_reports.each do |kpi_report|
        ...
        if kpi_report.document_folder_id.present?
          FolderLlmReportJob.perform_now(kpi_report.document_folder_id, user_id, "KpiReport",
                                         report_template_name:, kpis:, apis:)
        end
      end
  ...
  def api_reports(fund, user_id, report_template_name, include_kpis: false, include_apis: false, start_date: nil, end_date: nil)
