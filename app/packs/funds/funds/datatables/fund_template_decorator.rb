class FundTemplateDecorator < TemplateDecorator # rubocop:disable Metrics/ClassLength
  ...
  def undrawn_comm_prior_notice_lp
    ...
    prior_lp_committment_amt = if last_call_before
                                 last_call_before.capital_remittances.where(capital_commitment_id: fund_commitments_lp.pluck(:id)).where(remittance_date: ..(@remittance_date - 1.day).end_of_day).last.committed_amount
                               else
  ...
  def undrawn_comm_prior_notice_gp
    ...
    prior_gp_committment_amt = if last_call_before
                                 last_call_before.capital_remittances.where(capital_commitment_id: fund_commitments_gp.pluck(:id)).where(remittance_date: ..(@remittance_date - 1.day).end_of_day).last.committed_amount
                               else
                                 @capital_remittance.committed_amount
                               end
