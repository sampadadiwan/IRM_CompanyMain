class FundsController < ApplicationController
  ...
  def report
    if params[:report].present? && params[:report] == "generate_reports"
      ...
      @fund_report ||= FundReport.find(fund_id: @fund.id, name: params[:report].split('/').last.camelize).last
      @bread_crumbs = { Funds: funds_path, "#{@fund.name}": fund_path(@fund), 'Fund Reports': fund_reports_path(fund_id: @fund.id), "#{params[:report].split('/').last.camelize}": report_fund_path(@fund, fund_report_id: @fund_report.id, report: params[:report].to_s) }
