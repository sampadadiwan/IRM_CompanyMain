class CapitalCall < ApplicationRecord
  ...
  def notify_capital_call
    capital_remittances.pending.each do |cr|
      cr.investor.approved_users.each do |user|
        CapitalRemittanceNotification.with(entity_id:, capital_remittance: cr, email_method: :notify_capital_remittance, msg: "New Capital Call: #{name}").deliver_later(user)
      end
    end
  end
  ...
  def reminder_capital_call
    capital_remittances.pending.each do |cr|
      cr.investor.approved_users.each do |user|
        CapitalRemittanceNotification.with(entity_id:, capital_remittance: cr, email_method: :reminder_capital_remittance, msg: "Reminder for Capital Call: #{name}").deliver_later(user)
      end
    end
  end
