class CapitalCommitment < ApplicationRecord
  ...
  validates :folio_id, :fund_close, :commitment_date, presence: true
  ...
  def self.ransackable_attributes(_auth_object = nil)
    %w[created_at updated_at folio_id commitment_date fund_close investor_name onboarding_completed percentage unit_type committed_amount collected_amount call_amount distribution_amount esign_emails folio_currency total_units_amount units_amount_minus_collected foreign_currency_commitment fund_id].sort
  end
  ...
  def committed_amount_cents_before(date)
    date = Date.local_parse(date) if date.is_a?(String)
