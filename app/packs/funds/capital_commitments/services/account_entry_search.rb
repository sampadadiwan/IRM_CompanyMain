class AccountEntrySearch
  class << self
    ...
    def search_ids(params, current_user)
      ...
      AccountEntryIndex.filter(terms: { entity_id: entity_ids }).query(query_string: { fields: AccountEntryIndex::SEARCH_FIELDS, query:, default_operator: 'and' }).per(1000).map(&:id)
    end
  end
end
