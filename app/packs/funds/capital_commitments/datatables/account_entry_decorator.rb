class AccountEntryDecorator < ApplicationDecorator
  ...
  def amount
    if object && object.name.include?("Percentage")
      "#{object.amount_cents} %"
    else
  ...
  def dt_actions
    links = []
    links << h.link_to('Show', h.account_entry_path(object), class: "btn btn-outline-primary")
    links << h.link_to('Edit', h.edit_account_entry_path(object), class: "btn btn-outline-success") if h.policy(object).update?
    h.safe_join(links, '')
  end
end
