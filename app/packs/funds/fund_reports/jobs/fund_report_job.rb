class FundReportJob < ApplicationJob
  ...
  def generate(fund_id, report_name, start_date, end_date, user_id, excel: false, single: false)
    if report_name.blank?
      ALL_REPORTS.each do |report_name|
        if report_name.casecmp?("SEBI Report") || report_name.casecmp?("SEBIReport")
      ...
      if single
        generate_partial_sebi_report(fund_id, report_name, start_date, end_date, user_id, single: true)
      else
        generate_partial_sebi_report(fund_id, report_name, start_date, end_date, user_id)
      end
    elsif report_name.casecmp?("SEBI Report") || report_name.casecmp?("SEBIReport")
      generate_sebi_report(fund_id, start_date, end_date, user_id)
    else
      reporter = "#{report_name}Job".constantize.new
      reporter.generate_report(fund_id, start_date, end_date)
  ...
  def generate_sebi_report(fund_id, start_date, end_date, user_id)
    FileUtils.cp(Rails.root.join("SEBI report.xls"), Rails.root.join("tmp/sebi_report.xls"))
  ...
  def generate_partial_sebi_report(fund_id, report_name, start_date, end_date, user_id, single: false)
    FileUtils.cp(Rails.root.join("#{report_name}.xls"), Rails.root.join("tmp/#{report_name}.xls"))
