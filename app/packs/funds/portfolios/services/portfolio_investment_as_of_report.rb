class PortfolioInvestmentAsOfReport
  ...
  def add_pis_tab(workbook)
    ...
    workbook.add_worksheet(name: "PortfolioInvestments") do |sheet|
      # Create the header row
      sheet.add_row(["Id", "Fund", "Portfolio Company", "Instrument", "Investment Date", "Quantity", "Instrument Currency", "Amount in Instrument Currency", "Fund Currency", "Amount", "Cost", "Cost Of Sold", "Cost of Remaining", "Transfer Amount", "FMV", "Realized Gain", "Unrealized Gain", "Sold Quantity", "Transfer Quantity", "Net Quantity Available", "Total Qty As Of Investment Date", "Notes"] + custom_headers + custom_calc_headers)
      ...
      @aggregate_portfolio_investments.includes(portfolio_investments: %i[fund investment_instrument]).find_each do |api|
        ...
        api.portfolio_investments.each do |pi|
          ...
          sheet.add_row [pi.id, pi.fund.name, pi.portfolio_company_name, pi.investment_instrument, pi.investment_date, pi.quantity, pi.investment_instrument&.currency&.to_s, pi.base_amount.to_d, pi.fund.currency.to_s, pi.amount.to_d, pi.cost.to_d, pi.cost_of_sold.to_d, pi.cost_of_remaining.to_d, pi.transfer_amount.to_d, pi.fmv.to_d, gain.to_d, unrealized_gain.to_d, pi.sold_quantity, pi.transfer_quantity, net_qty, pi.quantity_as_of_date, pi.notes] + custom_field_values + custom_calc_values
        end
      end
    end
  end
