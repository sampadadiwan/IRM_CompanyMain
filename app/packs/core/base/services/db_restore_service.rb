...
class DbRestoreService # rubocop:disable Metrics/ClassLength
  ...
  def run!(instance_name: INSTANCE_NAME, stop_instance_flag: true)
    ...
    if result
      EntityMailer.with(subject: "#{ENV.fetch('BASE_DOMAIN', nil)}: DB Check PASSED", msg: { process: "DB RESTORE CHECK", result: "PASSED", message: msg, time_taken: "#{(total_duration / 60).round(1)} minutes" }).notify_info.deliver_now
    else
      raise msg
    end
  end
      ...
      File.open("log/db_restore_#{sanitized_ip}_#{timestamp}.log", "wb") do |f|
        ssh.open_channel do |channel|
          channel.exec("sudo bash #{REMOTE_SCRIPT_PATH} restore_primary") do |_ch, success|
            unless success
              msg = "❌ Failed to execute command. action: run_remote_script"
              f.write msg
  ...
  def validate_timestamp_presence(timestamp)
    if timestamp.blank?
      msg = "✗ Failed: Empty timestamp. action: validate_timestamp_presence"
      Rails.logger.debug msg
      raise msg
    end
    true
  ...
  def parse_timestamp(timestamp)
    ...
    msg = "✗ Failed: Invalid timestamp format '#{timestamp}'. action: parse_timestamp"
    Rails.logger.debug { msg }
    raise msg
  end
