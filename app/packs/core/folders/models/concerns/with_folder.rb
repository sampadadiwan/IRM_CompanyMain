module WithFolder
  ...
  def grant_access_rights_to_investor(document)
    if %w[CapitalCommitment CapitalRemittance CapitalDistributionPayment InvestorKyc IndividualKyc NonIndividualKyc].include?(self.class.name) && document.id.present?
